...
- **CORS Enabled**: Ready to connect with frontend web applications.
...
- **Response**:
  ...
  ```
  *(If the Gemini API is slow or offline, `source` returns `"fallback"` with reliable pre-generated questions so the application never hangs or crashes).*
